...
graph TD
    ...
    subgraph "Network Communication"
        Proxy[Proxy Server<br>(Listens on /api/v1/responses)]
        LLM[Local LLM Server<br>(e.g., Llama.cpp<br>/v1/chat/completions)]
    end

    HA -- 1. Request (Input, Tools)<br>/api/v1/responses --> Proxy;
    Proxy -- 2. Transform Request<br>(Format Tools, Set stream=false) --> Proxy;
    Proxy -- 3. Forward Transformed Request<br>/v1/chat/completions --> LLM;
    LLM -- 4. Process & Send<br>Complete Response<br>(Text OR Tool Calls) --> Proxy;
    Proxy -- 5. Receive & Parse<br>Complete Response --> Proxy;
    Proxy -- 6. Generate Fake SSE Stream<br>(Simulates OpenAI Streaming) --> Proxy;
    Proxy -- 7. Send Fake SSE<br>Stream to HA --> HA;
    HA -- 8. Parse Stream &<br>Display/Execute Tools --> HA;
